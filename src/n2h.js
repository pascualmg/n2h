...
function n2h(numberStr) {
    ...
    return parseRecursively (numberStr, periodLenght, initialWeight, periodTranslator, blockWeightTranslator).trim();
}
...
function parseRecursively(block, long, pesoAnterior, blockTranslator, weightTranslator) {
    ...
    //end recurse when
    if (isTheLastBlock) {
        return blockTranslator(miBlock, miPeso)+ ' ' + weightTranslator(miPeso) + ' ';
    }

    miPeso = pesoAnterior++;
    return parseRecursively (restante, long, miPeso + 1, blockTranslator, weightTranslator) + blockTranslator(miBlock,miPeso) + ' ' + weightTranslator(miPeso) + ' ';
}
